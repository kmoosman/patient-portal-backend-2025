...
export const createAttachment = async (req, res) => {
  ...
  if (!isAdmin) {
    return res
      .status(403)
      .json({ error: "You do not have permission to add an attachment for this user" });
  }
  ...
  try {
    ...
    const fileUpload = await createAttachmentService(urlToSave, patientId, data);
    ...
    const diagnosesToLink = (typeof data.diagnoses === 'string' && data.diagnoses != 'undefined') ? JSON.parse(data.diagnoses) : []
    //if there are diagnoses to link, link them
    if (diagnosesToLink.length > 0) {
      diagnosesToLink.forEach(async (diagnosis) => {
        await linkAttachmentToDiagnosisService(fileUpload.id, diagnosis.value, patientId);
      }
      );
    }

    const providersToLink = (typeof data.providers === 'string' && data.providers != 'undefined') ? JSON.parse(data.providers) : [];
    //if there are providers to link, link them
    if (providersToLink.length > 0) {
      providersToLink.forEach(async (provider) => {
        await linkAttachmentToProviderService(fileUpload.id, provider.value, patientId);
      }
      );
    }

    const institutionsToLink = (typeof data.institutions === 'string' && data.institutions != 'undefined') ? JSON.parse(data.institutions) : [];
    //if there are institutions to link, link them
    if (institutionsToLink.length > 0) {
      institutionsToLink.forEach(async (institution) => {
        await linkAttachmentToInstitutionService(fileUpload.id, institution.value, patientId);
      }
      );
    }
    const interventionsToLink = (typeof data.interventions === 'string' && data.interventions != 'undefined') ? JSON.parse(data.interventions) : [];
    ...
    if (interventionsToLink.length > 0) {
      interventionsToLink.forEach(async (intervention) => {
        await linkAttachmentToInterventionService(fileUpload.id, intervention.value, patientId);
      }
      );
    }

    const imagingToLink = (typeof data.imaging === 'string' && data.imaging != 'undefined') ? JSON.parse(data.imaging) : [];
    //if there are imaging to link, link them
    if (imagingToLink.length > 0) {
      imagingToLink.forEach(async (imaging) => {
        await linkAttachmentToImagingService(fileUpload.id, imaging.value, patientId);
      }
      );
    }

    const medicationsToLink = (typeof data.medications === 'string' && data.medications != 'undefined') ? JSON.parse(data.medications) : [];
    //if there are medications to link, link them
    if (medicationsToLink.length > 0) {
      medicationsToLink.forEach(async (medication) => {
        await linkAttachmentToMedicationService(fileUpload.id, medication.value, patientId);
      }
      );
    }
    res.json(fileUpload);
